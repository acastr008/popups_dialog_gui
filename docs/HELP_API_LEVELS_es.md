...
### Demo 1 — `demo_popuphelp_embebido.py`
...
### Demo 2 — `demo_showhelpoverlay_directo.py`
- Desde el popup principal, botón **Ayuda (Overlay)** → llama directamente a `ShowHelpOverlay(display, ...)`.
- Visualmente se parece mucho porque el overlay se dibuja sobre el frame congelado (donde ya estaba el popup), pero el **mecanismo** es distinto:
  - aquí la ayuda depende del **display** y de teclas de salida,
  - no hay “ventana de ayuda” del sistema de popups (a menos que lo construyas tú).
